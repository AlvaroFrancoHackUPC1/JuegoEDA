...
#define distHuirV 5
  ...
  vector<int> asignar_grupos() {
    ...
    for (int i = 0; i < n; ++i) {
      ...
      for (int g = 0; g < num_grupos; ++g) {
        if (conteos_grupos[g] < tam_grupo &&
            sumas_grupos[g] + valor <= suma_objetivo) {
  ...
  struct CompareLibWiz {
    bool operator()(const LibWiz& a, const LibWiz& b) const {
      if (a.dist != b.dist) return a.dist > b.dist;  // primero min dist
      return a.puntos < b.puntos;                    // luego max cas si dist ==
    }
  };

  // Devuelve la puntuación de esa casilla haciendo una minibusqueda

  int miniBusquedas(int wiz_id, Pos p, int distmax) {
    if (distmax == 0) return 0;
    bool fantasma = (wiz_id == ghost(me()));
    vector<Dir> movement = fantasma ? fdirs : wdirs;
    vector<vector<bool>> casVistas(board_rows(),
                                   vector<bool>(board_cols(), false));
    ...
    if (totalMagicStrength != 0) factorStrenght = float(magic_strength(me()) / totalMagicStrength * 10);
    ...
    while (!pendientes.empty()) {
      ...
      p = front.p;
      puntos = front.puntos;
      // Evaluar celdas con libros
      if (cell(p).book)
        puntos +=
            book_magic_strength() * factorStrenght;  // Alto valor para libros

      // Evaluar control de celdas
      if (cell(p).owner != -1 && cell(p).owner != me())
        puntos +=
            points_per_owned_cell() * factorScore;  // Controlado por un enemigo
      ...
      if (cell(p).id != -1) {
        ...
        if (cell(p).owner != me()) {
          if (unit_info.type == Wizard && !unit_info.is_in_conversion_process()) {
            ...
            puntos += points_for_converting_wizard()* probganar;  // Penalización por magos enemigos
          }
        }
      }

      for (int i = 0; i < int(movement.size()) && front.dist + 1 < distmax;
           ++i) {
        Pos pm = p + movement[i];
        if (celdaValida(pm) && !casVistas[pm.i][pm.j]) {
    ...
    //cerr << "puntos de la busqueda " << puntos << endl;
    return puntos;
  }
  ...
  void BFS(int wiz_id, queue<int>& bfsqueue, bool simple) {
    ...
    for (int i = 0; i < int(movement.size()); ++i) {
      Pos pm = p + movement[i];
      if (celdaValida(pm) && !casVistas[pm.i][pm.j]) {
        pendientes.push(LibWiz{1, pm, movement[i], wiz_id, miniBusquedas(wiz_id, p, 4), 1});
        casVistas[pm.i][pm.j] = true;
      }
    }
    while (!pendientes.empty()) {
      ...
      for (int i = 0; i < int(movement.size()); ++i) {
        Pos pm = p + movement[i];
        if (celdaValida(pm) && !casVistas[pm.i][pm.j]) {
          if (front.cooldown % (front.dist+1) == 0) pendientes.push( LibWiz{front.dist + 1, pm, front.mov, wiz_id, front.puntos + miniBusquedas(wiz_id, p, front.cooldown+1), front.cooldown+1});
          else pendientes.push( LibWiz{front.dist + 1, pm, front.mov, wiz_id, front.puntos, front.cooldown});
          casVistas[pm.i][pm.j] = true;
        }
      }
      ...
      if (cell(p).book ||
          (cell(p).id != -1 && cell(p).owner == me() &&
           unit(cell(p).id).is_in_conversion_process() &&
           unit(cell(p).id).rounds_for_converting() > front.dist &&
           !fantasma) ||
          (cell(p).id != -1 && cell(p).owner != me() &&
           ((magic_strength(me()) > 2 * magic_strength(cell(p).owner) ||
             (round() >= 100 &&
              2 * magic_strength(me()) > magic_strength(cell(p).owner))) ||
            (unit(cell(p).id).type == Ghost &&
             unit(cell(p).id).rounds_pending < 10)) &&
           !fantasma)) {
        if (simple) {
    ...
    // Si ha llegado auí significa que no a encontrado a por que ir en e se caso
    // que vaya a lo mas cercano;
  ...
  bool BFSHuirV(int wiz_id) {
    ...
    for (int i = 0; i < int(movement.size()); ++i) {
      ...
      if (celdaValida(pm) && !casVistas[pm.i][pm.j]) {
        int dist_next =
            abs(pos_voldemort().i - pm.i) + abs(pos_voldemort().j - pm.j);
    ...
    while (!pendientes.empty()) {
      ...
      for (int i = 0; i < int(movement.size()); ++i) {
        ...
        if (celdaValida(pm) && !casVistas[pm.i][pm.j]) {
          ...
          if (dist_next > distact) {
            pendientes.push(
                LibWiz{front.dist + 1, pm, front.mov, wiz_id,
                       front.puntos + (cell(pm).owner != me() ? 1 : 0)});
            casVistas[pm.i][pm.j] = true;
          }
        }
      }
  ...
  void atacarcerca(Unit wiz) {
    ...
    for (int i = 0; i < int(wdirs.size()); ++i) {
      if (celdaValida(p + wdirs[i]) && cell(p + wdirs[i]).id != -1) {
        ...
        if ((setWiz.find(pwiz.id) == setWiz.end()) ||
            pwiz.is_in_conversion_process()) {
          move(wiz.id, wdirs[i]);
          // cerr << "soy: " << wiz.id  << " estoy: " << p << " ataco: " <<
          // p+wdirs[i] << endl;
          return;
        }
      }
  ...
  virtual void play() {
    ...
    // Tirar hechizo
    if (round() > 50 && unit(ghostid).rounds_pending == 0)
      spell(ghostid, asignar_grupos());
    ...
    for (int i = 0; i < int(wids.size()); ++i) {
      ...
      // Huir
      bool movido = false;
      if (abs(posV.i - wiz.pos.i) <= distHuirV &&
          abs(posV.j - wiz.pos.j) <= distHuirV)
        movido = BFSHuirV(wiz.id);
...
/*
 vector<vector<bool>> casVistas(board_rows(), vector<bool>(board_cols(),
 false)); queue<LibWiz> pendientes; Pos p = unit(wiz_id).pos;
 casVistas[p.i][p.j] = true;
 pair<int,int> distini = distV(p);

 for (int i = 0; i < int(wdirs.size()); ++i) {
   Pos pm = p + wdirs[i];
   if (celdaValida(pm) && !casVistas[pm.i][pm.j] && ((distV(pm).first >
 distini.first && distV(pm).second >= distini.second) || (distV(pm).first >=
 distini.first && distV(pm).second > distini.second))) {
 ...
 while (!pendientes.empty()) {
   ...
   for (int i = 0; i < int(wdirs.size()); ++i) {
     ...
     if (celdaValida(pm) && !casVistas[pm.i][pm.j] && ((distV(pm).first >
 distini.first && distV(pm).second >= distini.second) || (distV(pm).first >=
 distini.first && distV(p).second > distini.second))) {
       pendientes.push(LibWiz{front.dist + 1, pm, front.mov, wiz_id});
       casVistas[pm.i][pm.j] = true;
     }
   }
